    ...
    <table class="info-table">
        <tr>
            <td class="bill-to-cell">
                ...
                <strong>Name:</strong> {{ $sale->customer ? $sale->customer->name : ($sale->customer_name ?? 'Mahidul Ratul') }}<br>
                <strong>Address:</strong> {{ $sale->customer ? $sale->customer->address : ($sale->customer_address ?? 'hhuh gyhj') }}<br>
                <strong>Mobile No.:</strong> {{ $sale->customer ? $sale->customer->phone : ($sale->customer_mobile ?? '01797778412') }}
            </td>
            ...
            <td class="totals-cell">
                <div class="totals-box">
                    <table class="totals-inner-table">
                        <tr>
                            <td>Sub Total:</td>
                            <td style="text-align: right;">BDT {{ number_format($sale->subtotal ?? ((float)$sale->unit_price * (int)($sale->quantity ?? 1)), 2) }}</td>
                        </tr>
                        @if($sale->discount_amount > 0)
                        <tr>
                            <td>Discount:</td>
                            <td style="text-align: right;">BDT -{{ number_format((float)$sale->discount_amount, 2) }}</td>
                        </tr>
                        @endif
                        <tr class="total-row">
                            <td>Total Amount:</td>
                            <td style="text-align: right;">BDT {{ number_format((float)$sale->total_amount, 2) }}</td>
                        </tr>
                        <tr>
                            <td>Paid Amount:</td>
                            <td style="text-align: right;">BDT {{ number_format((float)$sale->paid_amount, 2) }}</td>
                        </tr>
                        @if((float)$sale->total_amount - (float)$sale->paid_amount > 0)
                        <tr>
                            <td>Due Amount:</td>
                            <td style="text-align: right;">BDT {{ number_format((float)$sale->total_amount - (float)$sale->paid_amount, 2) }}</td>
                        </tr>
                        @endif
                    </table>
                </div>
    ...
    <div class="signature-section">
        <table class="signature-table">
            <tr>
                <td class="signature-cell">
                    ...
                    <small style="color: #6c757d;">{{ $sale->customer->name ?? 'Customer' }}</small>
                </td>
        ...
        <table class="support-table">
            <tr>
                ...
                <td class="support-cell">
                    <div class="support-title">Warranty & Service</div>
                    All products come with manufacturer warranty. Keep this invoice for warranty claims and future reference.
                </td>
            </tr>
        </table>
